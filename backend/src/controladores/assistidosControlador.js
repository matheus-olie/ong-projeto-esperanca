...
async function cadastrarAssistido(req, res) {
    try {
        ...
        if (!nome || !cpf || !idade || !escola || !serie || !responsavel || !telefone) {
            return res.status(400).json({
        ...
        const [resultado] = await conexao.query(
            'INSERT INTO tbl_assistidos(nome,cpf,idade,escola,serie,responsavel,telefone)VALUES (?, ?, ?, ?, ?, ?, ?)',
            [nome, cpfLimpo, idade, escola, serie, responsavel, telefoneLimpo]
        );
...
async function editarAssistido(req, res) {
    try {
        ...
        const [resultado] = await conexao.query(
            'UPDATE tbl_assistidos SET nome = ?, cpf = ?, idade = ?, escola = ?, serie = ?, responsavel = ?, telefone = ? WHERE id = ?',
            [nome, cpfLimpo, idade, escola, serie, responsavel, telefoneLimpo, id]
        );
...
async function excluirAssistido(req, res) {
    try {
        ...
        if (error.code === 'ER_ROW_IS_REFERENCED_2') {
            return res.status(400).json({
                mensagem: 'Não é possível excluir o assistido, pois existem participações vinculadas a ele.'
            });
        }
